// src/pages/admin/AdminSeasonings.tsx
...
import type { DbSeasoning } from "@/lib/supabase";

const ALLERGEN_OPTIONS = ["milk", "eggs", "gluten", "soy", "sesame", "peanuts", "tree-nuts", "shellfish", "fish", "mustard"];

const emptySeasoning = (): Omit<DbSeasoning, "id" | "created_at" | "updated_at"> => ({
  name: "", description: "", level: 0, image: "", allergens: [],
});

const AdminSeasonings = () => {
  const { request } = useAdminApi();
  const [items, setItems] = useState<DbSeasoning[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<Partial<DbSeasoning> | null>(null);
  ...
  // Supabase Realtime subscription for seasonings
  useEffect(() => {
    const channel = supabase
      .channel('public:seasonings')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'seasonings' }, payload => {
        // Refetch seasonings on any change
        void load();
      })
      .subscribe();
    return () => {
  ...
  const handleSave = async () => {
    ...
    setEditing(null);
    if (isNew) {
      toast({ title: "Seasoning added", description: "A new seasoning was created successfully." });
    }
    void load();
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    toast({ title: "Seasoning deleted", description: "The seasoning was removed successfully." });
    // No need to call load() here, realtime will update
  };

  const setField = <K extends keyof DbSeasoning>(key: K, value: DbSeasoning[K]) =>
  ...
  return (
    <AdminLayout>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-serif text-2xl text-foreground">Seasonings</h1>
        <Button onClick={() => setEditing(emptySeasoning())} size="sm">
          <Plus size={16} className="mr-1" /> Add Seasoning
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((item) => (
            <div key={item.id} className="border border-primary/20 rounded-xl p-4 bg-black/40 space-y-2">
              {item.image && (
                <img src={item.image} alt={item.name} className="h-24 w-full object-cover rounded-lg" />
              )}
              <h3 className="font-semibold">{item.name}</h3>
              <p className="text-sm text-muted-foreground line-clamp-2">{item.description}</p>
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">Heat: {item.level}/10</span>
          ...
          {items.length === 0 && (
            <p className="text-muted-foreground col-span-3 py-8 text-center">No seasonings yet. Add one!</p>
          )}
        </div>
      )}
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg max-h-[90vh] overflow-y-auto bg-background border border-primary/30 rounded-xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-serif text-xl">{editing.id ? "Edit Seasoning" : "New Seasoning"}</h2>
            ...
            <div className="space-y-3">
              ...
              <ImageUpload value={editing.image ?? ""} onChange={(url) => setField("image", url)} label="Seasoning Image" />
...
export default AdminSeasonings;
